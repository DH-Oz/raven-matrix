...
| AC5.1 (rotation) | Faithful rotation unit: a feature's `rotation` is degrees (mirrors Java `getRotation() % 360`) and is emitted directly as the SVG `rotate(θ)` argument, never double-converted. | unit (fitness function) | `tests/test_render_svg_cells.py` | 6 | A non-zero-rotation feature (e.g. 90°) emits `rotate(90)` exactly — not `rotate(116.6)` / `rotate(5156)`. Tripwire for the radians↔degrees erosion fixed during the Phase-6 coherence review (all other fixtures use `rotation=0`, which masks the unit). |
