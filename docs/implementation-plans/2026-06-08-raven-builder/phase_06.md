...
- Transform: emit `transform="translate({px} {py}) rotate({rot}) scale({s}) translate({-px} {-py})"` where `rot = feature.rotation` emitted **directly** and `s = feature.scale`. This reproduces the Java "scale+rotate about position" order. **[Corrected during Phase-6 coherence review — this note originally read "deg = degrees(feature.rotation) (Java rotation is radians → SVG degrees)", which was wrong: the model stores rotation in DEGREES (mirroring Java `getRotation() % 360`); Java converts deg→rad only because `AffineTransform.rotate()` needs radians; SVG `rotate()` takes degrees, so the value is emitted directly. See ADR-0001 and the AC5.1 rotation fitness function.]**
