...
- **Hand-derived table (AC2.1) is the HARD gate**, built independently from the Matzen naming convention + the published codes **+ the published PNGs for genuinely ambiguous cases** (DR6's independent reference frame) — never read from the Java labeller. It is created **wholly in Phase 5** (the design's "begun in Phase 4" wording is superseded — Phase 4 does direct structural inspection, no label-based table; this was a plan-evolution decision, not a numbered design Decision Record).
